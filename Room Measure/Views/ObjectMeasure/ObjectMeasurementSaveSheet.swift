...
struct ObjectMeasurementSaveSheet: View {
        ...
        NavigationStack {
            Form {
                Section(header: Text(LocalizedKey.arResult.localized)) {
                    HStack {
                        Text(LocalizedKey.roomMeasurements.localized).foregroundColor(.secondary)
                        Spacer()
                        Text(UnitHelper.size(measurement.width, measurement.height))
    ...
    private func save() {
        ...
        let userNotes = measurementNotes.trimmingCharacters(in: .whitespaces)
        room.measurements.append(SingleMeasurement(
            name: name,
            distance: measurement.width,
            objectHeight: measurement.height,
